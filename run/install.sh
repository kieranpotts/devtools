...
# eg `/path/to/tools/run/install.sh`.
file_path=$(readlink -f "$0")

# Absolute path to this script's parent directory,
# eg `/path/to/tools/run`.
...
# Absolute path to the user's root "dev" directory, which is
# expected to be two levels above the "tools" repo root.
