...
TrueType files for these fonts are included in the `./vendor/fonts` directory and need to be installed manually.
...
NOTE: This step is required only if Settings Sync is not enabled in VS Code.

In VS Code, go to File > Preferences > Profile > Profiles. Click the dropdown next to the New Profile button. Click Import Profile. Choose Select File. Select one of the `*.code-profile` files in `./etc/vscode`.
...
* {link-maple-font}[*Maple Font*]: Choose the default, unhinted TTF files with ligatures (ie. with merged characters_, from the repository's releases page. (The unhinted versions are optimized for high-resolution displays. The "normal" variant is another option. It is less opinionated than the "default" version, and is a little less characterful for it.)
