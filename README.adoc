...
==== 3. Create a symlink to the dictionary file

Again running Windows Powershell in administrator mode, execute the following command, changing the filesystem paths as required. This creates a symlink to `./var/dictionary.txt` in this repository from the same path in your home directory. This path is referenced from `./etc/vscode/settings.json` and is used by the Code Spell Checker extension as a custom dictionary.

.VS Code / VS Codium
[source]
----
New-Item -ItemType SymbolicLink -Path "C:\Users\[User]\var\dictionary.txt" -Target "C:\path\to\devtools\var\dictionary.txt" -Force
----
